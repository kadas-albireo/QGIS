...
QgsReportOrganizerWidget::QgsReportOrganizerWidget( QWidget *parent, QgsLayoutDesignerInterface *designer, QgsReport *report )
...
void QgsReportOrganizerWidget::removeSection()
{
  ...
  {
    if ( mDesigner->layout() == section->header() || mDesigner->layout() == section->footer() )
    ...
    if ( section->type() == QLatin1String( "SectionFieldGroup" ) )
    {
      ...
      if ( fieldGroup->body() == mDesigner->layout() )
        mDesigner->setCurrentLayout( nullptr );
    }
    if ( section->type() == QLatin1String( "SectionLayout" ) )
    {
      ...
      if ( sectionLayout->body() == mDesigner->layout() )
        mDesigner->setCurrentLayout( nullptr );
    }
